...
// gather folders under Fotos motos
...
for (const folder of Object.keys(imgsByFolder)){
  ...
  // attempt to find best matching moto by name tokens
  let best = null;
  let bestScore = 0;
  list.forEach(m => {
  ...
  if (best && bestScore > 0){
    ...
    try{
      ...
      // generate thumbnail if sharp available
      if (sharpAvailable){
        const thumbPath = path.join(imagesDir, `thumb-${best.id}.jpg`);
        // create thumbnail and handle errors; collect promises so we can await them later
        const p = sharp(dest).resize(640,384,{fit:'cover'}).jpeg({quality:80}).toFile(thumbPath)
          ...
          .catch(async err => {
            ...
            // try Jimp fallback for unsupported formats
            if (jimpAvailable){
              try{
...
Promise.all(thumbPromises)
  ...
  .catch(err => {
    // still write metadata even if thumbnail generation had unexpected error
